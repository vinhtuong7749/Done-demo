...
class IngredientDetailCubit extends Cubit<IngredientDetailState> {
  ...
  Future<void> loadIngredientDetails({
    ...
    try {
      if (_nguyenLieuService != null) {
        final response = await _nguyenLieuService!.getNguyenLieuDetail(
          maNguyenLieu,
        );
        ...
        final sellers =
            response.sellers.data.map((seller) {
              ...
              final aRank = a.isAvailable ? 0 : (a.isMoCua ? 1 : 2);
              final bRank = b.isAvailable ? 0 : (b.isMoCua ? 1 : 2);
              return aRank.compareTo(bRank);
            });

        // Tính tổng số lượng còn (chỉ tính những gian hàng còn hàng)
        final totalStock = sellers
            .where((s) => s.conHang)
            .fold<int>(0, (sum, seller) => sum + seller.soLuongBan);
        ...
        debugPrint(
          '✅ Loaded ingredient detail: ${detail.tenNguyenLieu} with ${sellers.length} sellers',
        );
        ...
        firstSeller ??= sellers.isNotEmpty ? sellers.first : null;
        ...
        emit(
          state.copyWith(
            ...
            description:
                'Có ${detail.soGianHang} gian hàng đang bán sản phẩm này',
      ...
      debugPrint('⚠️ Lỗi khi fetch chi tiết nguyên liệu: $e');
      emit(
        state.copyWith(
          isLoading: false,
  ...
  void _loadMockData(
    ...
    emit(
      state.copyWith(
        ...
        soldCount: 59,
        description:
            'Sản phẩm tươi ngon, được nhập khẩu trực tiếp từ các nông trại uy tín.',
  ...
  Future<void> addToCart() async {
    ...
    if (!seller.isMoCua) {
      emit(
        state.copyWith(
          ...
          lastCartActionMessage: 'Cửa hàng đang không hoạt động',
        ),
      );
    ...
    if (!seller.conHang) {
      emit(
        state.copyWith(
          lastCartActionSuccess: false,
          lastCartActionMessage: 'Sản phẩm hiện đã hết hàng',
        ),
      );
      Future.delayed(const Duration(seconds: 2), () {
        if (!isClosed) {
  ...
  void clearCartActionMessage() {
    emit(
      state.copyWith(lastCartActionMessage: null, lastCartActionSuccess: null),
    );
  }
  ...
  void buyNow(BuildContext context) {
    ...
    if (!state.selectedSeller!.isMoCua || !state.selectedSeller!.conHang) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            !state.selectedSeller!.isMoCua
    ...
    debugPrint(
      '🛍️ [BUY NOW] Navigating to payment with ingredient ${state.ingredientName} at ${state.selectedSeller!.tenGianHang}',
    );
  ...
  /// Chat with selected shop
  Future<void> chatWithShop(BuildContext context) async {
    final seller = state.selectedSeller;

    if (seller == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Vui lòng chọn gian hàng trước khi trò chuyện'),
        ),
      );
      return;
    }
  ...
  void selectSeller(Seller seller) {
    debugPrint(
      '🏪 [SELECT SELLER] Selecting: ${seller.maGianHang} - ${seller.tenGianHang}',
    );

    if (!seller.isAvailable) {
      emit(
        state.copyWith(
          errorMessage: !seller.isMoCua
              ? 'Cửa hàng đang không hoạt động'
              : 'Sản phẩm ở gian hàng này đã hết hàng',
        ),
      );
      Future.delayed(const Duration(seconds: 2), () {
        if (!isClosed) {
    ...
    debugPrint(
      '✅ Đã chọn gian hàng: ${seller.tenGianHang} (${seller.maGianHang}) - ${seller.price}',
    );
  ...
  Future<void> loadReviewsForSeller(String maGianHang) async {
    ...
    try {
      ...
      if (response.success) {
        debugPrint(
          '✅ [REVIEWS] Loaded ${response.items.length} reviews, avg: ${response.avg}',
        );
        emit(
          state.copyWith(
  ...
  void increaseQuantity() {
    ...
    if (state.quantity < maxQuantity) {
      ...
      debugPrint('➕ Số lượng: ${state.quantity}');
    } else {
      emit(state.copyWith(errorMessage: 'Chỉ còn $maxQuantity sản phẩm'));
      Future.delayed(const Duration(seconds: 2), () {
        if (!isClosed) {
          emit(state.copyWith(errorMessage: null));
        }
  ...
  Future<void> _loadRandomProducts(String currentMaNguyenLieu) async {
    ...
    try {
      ...
      final relatedProducts = relatedList
          .map(
            (p) => RelatedProduct(
      ...
      final recommendedProducts = recommendedList
          .map(
            (p) => RelatedProduct(
              maNguyenLieu: p.maNguyenLieu,
              name: p.tenNguyenLieu,
      ...
      debugPrint(
        '✅ [RANDOM] Loaded ${relatedProducts.length} related, ${recommendedProducts.length} recommended',
      );
